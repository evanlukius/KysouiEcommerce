...
class CartController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        ...
        try {
            ...
            $transaction->status = $request->mode == 'card' ? "completed" : "pending";
            $transaction->save();
            ...
            return back()->withErrors(['error' => 'An error occurred while processing your order.']);
        }
    }
    ...
    public function setAmountForCheckout()
    {
        if (Cart::instance('cart')->count() > 0) {
            ...
            session()->forget('checkout'); // Clear session if cart is empty
        }
    }
    
    public function confirmation()
    {
        return view('order-confirmation'); // Ensure this route is correctly configured
    }

    public function showOrderConfirmation($orderId)
    {
        $order = Order::with('transaction', 'orderItems.product')->findOrFail($orderId);
